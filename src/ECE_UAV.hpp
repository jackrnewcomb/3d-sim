...
class ECE_UAV {
 public:
  ...
  /**
   * @brief Starts the UAV execution by assigning the std::thread worker to
   * threadFunction
   */
  void start();
   ...
   * @returns mutex
   */
  std::mutex& getMutex() { return mMtx; }
  ...
  /**
   * @brief The general physics update loop. Performs all calculations to update
   * force, acceleration, velocity, and position
   *
   * @param The time delta
   * @param How much time has elapsed since simulation start (required for 5
   * second wait period)
   */
 ...
 private:
  /**
   * @brief A random point generator that outputs a random point on the surface
   * of the sphere. Useful in picking paths for the UAVs
   */
  ...
  /**
   * @brief Ensures the magnitude of any vector doesn't exceed a maximum
   * (helpful for clamping max ascent velocity and max force)
   *
   * @param Any vector
  ...
  glm::vec3 fixMagnitude(const glm::vec3& v, float maxLen) {
    ...
    if (len2 <= maxLen * maxLen) return v;
  ...
  bool mInSphereMode =
      false;  // indicates whether the uav has reached the surface of the sphere
