...
void ECE_UAV::update(float dt, float elapsedSinceStart) {
  ...
  if (!mInSphereMode) {
    ...
    // Get the desired velocity, force, and acceleration vectors to ascend
    // toward the sphere
    glm::vec3 dir = (distFromSphere > 1e-6f) ? (toSphere / distFromSphere)
                                             : glm::vec3(0.0f, 0.0f, 1.0f);
    glm::vec3 desiredVelocity = dir * mAscentSpeed;
    glm::vec3 desiredAcceleration =
        (desiredVelocity - getVelocity()) / std::max(dt, 1e-4f);
    glm::vec3 reqForce = mMass * desiredAcceleration -
                         gravityForce;  // gravity is added separately
    ...
    // Now that we're in sphere mode, our basic strategy is to randomly select a
    // point on the sphere to travel to, and a velocity between 2-10 m/s. We
    // will travel toward that point, with that velocity, until we reach it,
    // then repeat
    ...
    // If we reached the point we were traveling to, get a new point and
    // velocity
    if (dist < 0.1f) {
  ...
  // Get new acceleration, position, and velocity from the force we just
  // calculated
  glm::vec3 newAcc = totalForce / mMass;
  glm::vec3 newPos =
      getPosition() + getVelocity() * dt + 0.5f * newAcc * dt * dt;
  glm::vec3 newVel = getVelocity() + newAcc * dt;

  // If we're in sphere mode, we need to make alterations to ensure the UAV
  // remains on the surface of the sphere
  if (mInSphereMode) {
